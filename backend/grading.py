...
def grade_player(stats: dict, win_probability: float) -> str:
    won = stats["win"]

    if won and win_probability >= 0.70:   return "S"
    if won and win_probability >= 0.55:   return "A"
    if won and win_probability >= 0.40:   return "B"  # won but stats were average
    if won:                                return "C"  # won but stats were poor / lucky
    if not won and win_probability >= 0.60: return "C" # unlucky loss, stats were good
    if not won and win_probability >= 0.45: return "D"
    return "F"